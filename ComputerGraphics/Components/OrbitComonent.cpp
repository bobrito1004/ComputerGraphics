...
OrbitComponent::OrbitComponent(Game* game, float radius, int segments,
...
void OrbitComponent::Initialize()
{
    ...
    hr = game->device_->CreateBuffer(&vertexBufDesc, &vertexData, &vertexBuffer_);

    D3D11_BUFFER_DESC constBufDesc = {};
    constBufDesc.ByteWidth = sizeof(Matrix);
    constBufDesc.Usage = D3D11_USAGE_DEFAULT;
    constBufDesc.BindFlags = D3D11_BIND_CONSTANT_BUFFER;
    hr = game->device_->CreateBuffer(&constBufDesc, nullptr, &constantBuffer_);
}
...
void OrbitComponent::Draw()
{
    Matrix worldViewProj =
        ...
        game->Camera->GetMatrix();

    game->context_->UpdateSubresource(
        constantBuffer_.Get(),
        0,
        nullptr,
        &worldViewProj,
    ...
    game->context_->VSSetShader(vertexShader_.Get(), nullptr, 0);
    game->context_->PSSetShader(pixelShader_.Get(), nullptr, 0);
    game->context_->IASetInputLayout(layout_.Get());
    game->context_->IASetPrimitiveTopology(D3D11_PRIMITIVE_TOPOLOGY_LINELIST);
    game->context_->VSSetConstantBuffers(0, 1, constantBuffer_.GetAddressOf());

    UINT stride = sizeof(Vertex);
    UINT offset = 0;
    game->context_->IASetVertexBuffers(0, 1, vertexBuffer_.GetAddressOf(), &stride, &offset);

    game->context_->Draw(vertexCount_, 0);
}
...
void OrbitComponent::SetCenter(const Vector3& center)
{
    if (center_ != center)
    {
        ...
        if (SUCCEEDED(hr)) {
            ...
            game->context_->Unmap(vertexBuffer_.Get(), 0);
        }*/
    }
}
